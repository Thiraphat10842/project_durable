...
const Frmheaderuser: FC = () => {
  ...
  async function fetchNotifications() {
    try {
      ...
      const newNotifications = Mydata.filter(
        (row: Notification) =>
          row.Reportproblem === "แล้วเสร็จ" && row.type === "0"
      );
      ...
      const filteredNotifications = newNotifications.filter(
        (notification) =>
          !removedNotifications.some(
            (removed: Notification) => removed.ID === notification.ID
          )
      );
  ...
  const markAsRead = (index: number) => {
    setNotifications((prev) => {
      ...
      let removedNotifications = JSON.parse(
        localStorage.getItem("removedNotifications") || "[]"
      );
          ...
          <ul className="navbar-nav float-end">
            ...
            <li className="nav-item dropdown">
              ...
              >
                ...
                <a className="dropdown-item" href="/User/FrmProfile">
